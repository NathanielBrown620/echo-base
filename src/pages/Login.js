...
function Login() {
  ...
  async function handleLogin() {
    ...
    localStorage.setItem('echobase_user', JSON.stringify({
      id: data.id,
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", backgroundColor: '#f0f4f8', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: '420px', padding: '0 1rem' }}>
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '12px', border: '0.5px solid #e2e8f0', padding: '2rem' }}>
          ...
          {error && (
            <div style={{ backgroundColor: '#fee2e2', color: '#991b1b', padding: '0.75rem 1rem', borderRadius: '6px', fontSize: '13px', marginBottom: '1rem' }}>
              {error}
            </div>
          )}
          ...
          <input
            ...
            style={{ width: '100%', padding: '0.6rem 0.75rem', borderRadius: '6px', border: '0.5px solid #e2e8f0', fontSize: '13px', color: '#1a2332', outline: 'none', boxSizing: 'border-box', marginBottom: '1rem' }}
          />
          ...
          <input
            ...
            style={{ width: '100%', padding: '0.6rem 0.75rem', borderRadius: '6px', border: '0.5px solid #e2e8f0', fontSize: '13px', color: '#1a2332', outline: 'none', boxSizing: 'border-box', marginBottom: '1.5rem' }}
          />

          <button
            onClick={handleLogin}
            disabled={loading}
            style={{ width: '100%', backgroundColor: '#29ABE2', color: 'white', border: 'none', padding: '0.7rem', borderRadius: '6px', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', fontSize: '14px', opacity: loading ? 0.7 : 1 }}>
            {loading ? 'Signing in...' : 'Sign In'}
          </button>
        </div>

        <div style={{ textAlign: 'center', marginTop: '1rem', fontSize: '12px', color: '#94a3b8' }}>
